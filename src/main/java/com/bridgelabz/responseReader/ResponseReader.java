...
public class ResponseReader {
	...
	/*--------------------------------to read metric name type from response------------------------------------*/
				...
				for (int l = 0; l < metricheader1.size(); l++) {
					...
					metricHeaderArrayList.add((String) metricElemnt.get("name"));//*getting name of particular event
				...
				for (int i = 0; i < rowarray.size(); i++) {
					...
					for (int k = 0; k < metricarray.size(); k++) {
						...
						for (int l1 = 0; l1 < valuesarray.size(); l1++) {
							...
							//System.out.println("metric"+metricHashMap.get(metricHeaderArrayList.get(l1)));
						}
						// adding into
